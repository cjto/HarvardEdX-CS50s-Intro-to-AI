...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        self.list_cells(self.mines, self.safes)
            ...
            for sentence2 in self.knowledge:
                if sentence1 != sentence2 and len(sentence1.cells) > 0 and len(sentence2.cells) > 0 and sentence1.cells.issubset(sentence2.cells):
                    ...
                    if len(newSet) > 0 and newCount > -1:
                        sentences.append(Sentence(newSet, newCount))

        for newSentence in sentences:
            if newSentence not in self.knowledge:                
                self.knowledge.append(newSentence)
                
        self.list_cells(self.mines, self.safes)

    def list_cells(self, mines, safes):
        newSafes = set()
        newMines = set()
        for sentence in self.knowledge:
            for safe in sentence.known_safes():
                if safe not in newSafes:
                    if safe not in safes:
                        newSafes.add(safe)

            for mine in sentence.known_mines():
                if mine not in newMines:
    ...
    def make_safe_move(self):
        ...
        """
        projectedMoves = self.safes - self.moves_made

        if projectedMoves:
            return random.choice(tuple(projectedMoves))
        else:
            return None
    ...
    def make_random_move(self):
        ...
        for i in range(0, self.height):
            for j in range(0, self.width):
                randomMove = (i, j)
                if randomMove not in self.mines:
                    if randomMove not in self.moves_made:
                        return randomMove
